...
import { DevvitMessage, BlocksToWebviewMessage, Payload, createDefaultPayloadBasedOnType } from '../shared';
...
export const useDevvitListenerAnimated = <T extends BlocksToWebviewMessage['type']>(eventType: T,set:SpringRef<any>) => {
  type Event = Extract<BlocksToWebviewMessage, { type: T }>;
  useEffect(() => {
    const messageHandler = (ev: MessageEvent<DevvitMessage>) => {
      if (ev.data.type !== 'devvit-message') {
